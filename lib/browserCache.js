...
export async function browserCacheFetch(
  ...
  ttlSeconds = 3600,
  {
    ...
    onBackgroundUpdate = null, // Callback for live updates
  } = {}
) {
  try {
    ...
    // VALID CACHE → return instantly ONLY IF IT HAS ITEMS
    if (!isExpired && hasCachedItems) {
      ...
      fetcher().then(async (fresh) => {
        ...
        if (hasData) {
          ...
          if (onBackgroundUpdate) {
            console.log("📡 [IDB] Triggering UI Update:", key);
