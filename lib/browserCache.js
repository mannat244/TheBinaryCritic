  ...
  try {
    ...
    if (
      ...
      fetcher().then(async (fresh) => {
        if (fresh && !fresh.error && (fresh.items?.length > 0 || (Array.isArray(fresh) && fresh.length > 0))) {
          await db.put("cache", {
            key,
            data: fresh,
            timestamp: Date.now(),
    ...
    while (attempt < retries) {
      try {
        ...
        if (fresh && !fresh.error && fresh.items?.length > 0) break; // SUCCESS: Must have items!
        if (Array.isArray(fresh) && fresh.length > 0) break; // Support plain array response
    ...
    const hasItems = fresh?.items?.length > 0 || (Array.isArray(fresh) && fresh.length > 0);
