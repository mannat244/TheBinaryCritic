...
import { Play, Loader2, Eye, Check } from "lucide-react";
...
export default function TvClient({ initialData }) {
    const { id } = useParams();
    const { data: session } = useSession();
    const { watched } = useMediaStore();
    const [tv, setTv] = useState(initialData || null);
    ...
    const fetchStats = async () => {
        const res = await fetch(
            `/api/stats?mediaId=${id}&mediaType=tv`
        );
    ...
    const fetchReviews = async () => {
        try {
            const res = await fetch(`/api/reviews?mediaId=${id}&mediaType=tv&sort=${sortBy}&page=${currentPage}&limit=10`);
            const data = await res.json();
    ...
    // Prefetch trailer when TV data is available
    useEffect(() => {
        if (tv) {
            fetchTrailer(tv);
        }
    ...
    const fetchTrailer = async (currentTv) => {
        setTrailerLoading(true);
        setTrailerId(null);

        // 1. Try to find a trailer in TMDB data
        const videos = currentTv.videos?.results || [];
        ...
        try {
            const trailerKey = `trailer_tv_${currentTv.id}`;
                ...
                async () => {
                    const res = await fetch("/api/trailer", {
                        ...
                        body: JSON.stringify({
                            ...
                            year: new Date(currentTv.first_air_date).getFullYear(),
                            type: "tv"
                        }),
                    });
                    return res.json();
    ...
    // Data Extractors
    const getCreator = () => tv.created_by?.map(c => c.name).join(", ") || "Unknown";
    const getCountry = () => tv.production_countries?.[0]?.name || "Unknown";
    const getLanguage = () => tv.spoken_languages?.[0]?.english_name || "Unknown";
    ...
    if (!tv) return <div className="text-white text-center mt-20">TV Show not found</div>;
    ...
    return (
      ...
      ----------------------------------------------------- */}
            <div className="relative w-full mt-14 h-[25vh] md:mt-0 md:h-screen overflow-hidden group">
                ...
                {tv.backdrop_path ? (
                    <Image
                        src={
                            tv.backdrop_path.startsWith('https://')
                                ? tv.backdrop_path
                ...
                <Dialog open={trailerOpen} onOpenChange={setTrailerOpen}>
                    <DialogContent className="min-w-[80vw] w-full bg-black border-white/10 p-0 overflow-hidden aspect-video">
                        <DialogHeader className="sr-only">
                            <DialogTitle>{tv.name} Trailer</DialogTitle>
                        </DialogHeader>
                        ...
                        {trailerLoading ? (
                            ...
                            <div className="w-full h-full flex flex-col items-center justify-center bg-neutral-900 text-neutral-400 gap-4">
                                ...
                                <Button
                                    variant="outline"
                                    onClick={() => window.open(`https://www.youtube.com/results?search_query=${encodeURIComponent(tv.name + " trailer")}`, "_blank")}
                                >
                                    Search on YouTube
                                </Button>
                            </div>
                    ...
                    <div className="max-w-7xl mx-auto px-6 w-full flex items-end gap-8">
                        ...
                        <div className="flex-1 mb-4 pl-6">
                            ...
                            <div className="grid grid-cols-4 gap-12 border-t border-white/5 pt-6">
                                <div>
                                    <p className="text-neutral-500 text-[10px] uppercase tracking-widest mb-2">Created By</p>
                                    <p className="text-white font-medium text-base">{getCreator()}</p>
                                </div>
                                <div>
            ...
            <div className="max-w-[85%] hidden xl:block mx-auto pb-10 bg-black">
                {/* Keywords (Desktop) */}
                {tv.keywords?.results?.length > 0 && (
                    <div className="mt-0 flex flex-wrap justify-evenly gap-2">
                        {tv.keywords.results.slice(0, 10).map((keyword) => (
                            <Badge
                                key={keyword.id}
                                variant="outline"
      ...
      ----------------------------------------------------- */}
            <div className="md:hidden px-6 relative z-10 pb-10 -mt-14">
                ...
                <div className="grid grid-cols-3 gap-4 border-y border-white/5 py-4 mb-6">
                    <div>
                        <p className="text-neutral-500 text-[10px] uppercase tracking-widest mb-1">Creator</p>
                        <p className="text-white font-medium text-sm truncate">{getCreator()}</p>
                    </div>
                    <div>
                ...
                <div className="flex flex-wrap justify-left  w-full gap-2">
                    {tv.keywords?.results?.slice(0, 8).map((keyword) => (
                        <Badge
                            key={keyword.id}
                            variant="outline"
            ...
            <div className="max-w-7xl mx-auto px-6 pb-20 pt-1">
                ...
                {tv.credits?.crew?.length > 0 && (
                    <div>
                        ...
                        <div className="grid grid-cols-3 md:grid-cols-3 lg:grid-cols-6 gap-4">
                            {tv.credits.crew
                                .filter(p => ["Executive Producer", "Director", "Writer"].includes(p.job))
                                .reduce((acc, current) => {
                ...
                <div className="mt-5 flex justify-start">
                    <div className="w-full max-w-xl">
                        <StreamingCard tmdbId={tv.id} type="tv" />
                    </div>
                </div>
                ...
                <div className="mt-4">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        <VerdictRadialChart stats={stats} voteAvg={tv.vote_average} voteCount={tv.vote_count} />
                ...
                {!userHasReviewed && (
                    <div className="mt-12">
                        ...
                        ) : !watched?.some(i => i.tmdbId === Number(id) && i.mediaType === "tv") ? (
                            <div className="bg-neutral-900/50 border border-white/5 rounded-2xl p-8 text-center">
                                ...
                                <p className="text-neutral-400 text-sm mb-6 max-w-md mx-auto">
                                    You need to mark this TV show as "Watched" before you can leave a review.
                                </p>
                            </div>
                        ) : (
                            <ReviewCard
